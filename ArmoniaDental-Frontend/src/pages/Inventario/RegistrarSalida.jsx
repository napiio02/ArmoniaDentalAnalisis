...
const RegistrarSalida = () => {
	...
	const stockDisponible = insumoOriginal?.stock_actual ?? 0;
	const cantidadValida = cantidad !== "" && !isNaN(cantidadNum) && cantidadNum > 0 && cantidadNum <= stockDisponible;
	const excedeStock = cantidad !== "" && !isNaN(cantidadNum) && cantidadNum > stockDisponible;
	const stockProyectado = insumoOriginal
		? Math.max(stockDisponible - (cantidadNum > 0 ? cantidadNum : 0), stockDisponible)
		: 0;
	const stockProyectadoReal = insumoOriginal
		? stockDisponible - (cantidadValida ? cantidadNum : 0)
		: 0;
	...
	const onChangeCantidad = (e) => {
		...
		} else if (Number(valor) > stockDisponible) {
			setErrorCantidad(`Stock insuficiente. Disponible: ${stockDisponible} ${insumoOriginal?.unidad || ""}`);
		} else {
			setErrorCantidad(null);
		}
	};
	...
	const handleSubmit = async (e) => {
		...
		setError(null);
		try {
			await insumoService.registrarSalida(id, cantidadNum, fecha);
			navigate("/inventario");
		} catch (err) {
			setError(err.response?.data?.error || "Error al registrar la salida. Intentá de nuevo.");
			setGuardando(false);
		}
	};
	...
	return (
		<div className="min-h-screen bg-[#f9f9ff] font-[Nunito_Sans,sans-serif] flex flex-col">
			...
			<header className="bg-white border-b border-[#bec8ce] px-8 py-4 flex items-center gap-3">
				...
				<span className="text-[#bec8ce] mx-1">/</span>
				<span className="text-sm font-semibold text-[#006686]">Registrar Salida</span>
			</header>
			...
			<div className="flex-1 flex items-start justify-center px-6 py-10">
				<div className="w-full max-w-md">
					...
					<div className="mb-8">
						<h2 className="text-[28px] font-bold text-[#151c27]">Registrar Salida</h2>
						<p className="text-sm text-[#3f484e] mt-1">Ingresá la cantidad utilizada para restarla del stock actual</p>
					</div>
					...
					<div className="bg-white border border-[#bec8ce] rounded-2xl p-8 shadow-sm">
						...
						<div className="bg-[#f0f3ff] rounded-xl p-4 mb-6 flex items-center gap-3">
							<div className="w-10 h-10 rounded-full bg-[#ffdad6]/60 border border-[#ba1a1a]/20 flex items-center justify-center text-[#ba1a1a]">
						...
						<form className="space-y-5" onSubmit={handleSubmit} autoComplete="off">
							...
							<div className="bg-[#f9f9ff] border border-[#bec8ce] rounded-xl px-4 py-3 flex items-center justify-between">
								<span className="text-xs font-semibold text-[#3f484e] uppercase tracking-wider">Stock disponible</span>
								<span className="text-lg font-bold text-[#151c27]">{stockDisponible} {insumoOriginal.unidad}</span>
							</div>
							...
							<div>
								...
								<input type="number" min="1" max={stockDisponible} placeholder="Ej: 5"
							...
							<div className={`rounded-xl px-4 py-3 flex items-center justify-between border ${
								cantidadValida
									? "bg-[#7dd3fc20] border-[#006686]/20"
									: "bg-[#f0f3ff] border-[#bec8ce]"
							}`}>
								<span className={`text-xs font-semibold uppercase tracking-wider ${cantidadValida ? "text-[#006686]" : "text-[#3f484e]"}`}>
									Nuevo stock proyectado
								</span>
								<span className={`text-lg font-bold ${cantidadValida ? "text-[#006686]" : "text-[#151c27]"}`}>
									{stockProyectadoReal} {insumoOriginal.unidad}
								</span>
							</div>
							...
							<div className="flex justify-end gap-3 pt-2">
								...
								<button type="submit" disabled={guardando || !cantidadValida}
									...
									{guardando ? (
										...
										<><span className="material-symbols-outlined text-[16px]">check</span>Registrar salida</>
									)}
								</button>
							</div>
						</form>

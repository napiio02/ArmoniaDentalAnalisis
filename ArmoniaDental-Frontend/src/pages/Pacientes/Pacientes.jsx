...
const Pacientes = () => {
	...
	useEffect(() => { setPaginaActual(1); }, [busqueda, estadoFiltro]);
	...
	const pacientesActuales = filtrados.slice(indicePrimero, indicePrimero + POR_PAGINA);
			...
			<main className="flex-1 h-screen overflow-y-auto p-8">
				<div className="max-w-screen-2xl mx-auto">
					...
					<div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
						<div>
							<h2 className="text-[28px] font-bold leading-[36px] text-[#151c27]">Pacientes</h2>
							<p className="text-sm text-[#3f484e] mt-1">Gestiona la información de los pacientes de la clínica</p>
						</div>
						...
						<div className="overflow-x-auto">
							{cargando ? (
								...
								<div className="flex items-center justify-center gap-3 py-12 text-[#ba1a1a] text-sm">
									...
									{error}
									<button onClick={cargarPacientes} className="underline font-semibold ml-2">Reintentar</button>
								</div>
							) : filtrados.length === 0 ? (
								<div className="text-center py-16">
									<span className="material-symbols-outlined text-5xl text-[#bec8ce] block mb-3">person_search</span>
									...
									<table className="w-full text-left border-collapse">
										<thead>
											<tr className="bg-[#f0f3ff] border-b border-[#bec8ce]">
												{["#", "Paciente", "Cédula", "Teléfono", "Correo", "Estado", "Acciones"].map((h) => (
										...
										<tbody className="divide-y divide-[#bec8ce]/40">
											{pacientesActuales.map((paciente, index) => (
												<tr key={paciente._id}
													...
													<td className="px-5 py-4 text-sm text-[#3f484e]">{indicePrimero + index + 1}</td>
													<td className="px-5 py-4">
														<div className="flex items-center gap-3">
															...
															<span className="text-sm font-semibold text-[#151c27]">{paciente.nombre}</span>
														</div>
													</td>
													<td className="px-5 py-4 text-sm text-[#3f484e] font-mono">{paciente.cedula || "—"}</td>
													<td className="px-5 py-4 text-sm text-[#3f484e]">{paciente.telefono || "—"}</td>
													<td className="px-5 py-4 text-sm text-[#3f484e]">{paciente.correo || "—"}</td>
													<td className="px-5 py-4">
														{paciente.activo ? (
															...
															<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#ffdad6] text-[#ba1a1a] border border-[#ba1a1a]/20">Inactivo</span>
														)}
													</td>
													<td className="px-5 py-4">
														<div className="flex items-center gap-2">
															<Link to={`/pacientes/${paciente._id}`}
																className="p-1.5 rounded border border-[#bec8ce] text-[#3f484e] hover:border-[#006686] hover:text-[#006686] transition-all" title="Ver">
																<span className="material-symbols-outlined text-[18px]">visibility</span>
															</Link>
															<Link to={`/pacientes/editar/${paciente._id}`}
									...
									{totalPaginas > 1 && (
										<div className="px-5 py-3 flex items-center justify-between bg-[#f0f3ff] border-t border-[#bec8ce]">
											<span className="text-xs font-semibold text-[#3f484e]">
												Mostrando {indicePrimero + 1} a {Math.min(indicePrimero + POR_PAGINA, filtrados.length)} de {filtrados.length} pacientes
											</span>
												...
												{Array.from({ length: Math.min(totalPaginas, 5) }, (_, i) => i + 1).map((n) => (
													<button key={n} onClick={() => setPaginaActual(n)}
														className={`w-8 h-8 rounded text-xs font-semibold flex items-center justify-center transition-colors ${
															paginaActual === n ? "bg-[#006686] text-white" : "hover:bg-[#dce2f3] text-[#3f484e]"
														}`}>
														{n}
													</button>
												))}
